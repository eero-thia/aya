...
fn new_static_bpf_map_def(section: &Section, map_flags: u32) -> bpf_map_def {
    bpf_map_def {
...
fn parse_map(section: &Section, name: &str) -> Result<Map, ParseError> {
    ...
    let (def, data) = if name == ".bss" || name.starts_with(".data") {
        let def = new_static_bpf_map_def(section, 0);
        (def, section.data.to_vec())
    } else if name.starts_with(".rodata") {
        let def = new_static_bpf_map_def(section, BPF_F_RDONLY);
        (def, section.data.to_vec())
    } else {
